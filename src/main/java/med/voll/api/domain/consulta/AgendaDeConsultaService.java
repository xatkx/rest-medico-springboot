...
public class AgendaDeConsultaService {
    ...
    public void agendar( DatosAgendarConsulta datos){
        ...
        if(datos.idMedico() != null && !medicoRepo.findById(datos.idMedico()).isPresent()){
            throw new RuntimeException("medioc invalido bro");
        }
        if(!pacienteRepo.findById(datos.idPaciente()).isPresent()){
            throw new RuntimeException("paciente invalido bro");
        }


        var medico = medicoRandom(datos);
    ...
    private Medico medicoRandom(DatosAgendarConsulta datos) {
        if(datos.idMedico() != null){
            return medicoRepo.getReferenceById(datos.idMedico());
        }
        if(datos.especialidad() == null){
            throw new RuntimeException("especialidad no puede ser null");
        }
        return medicoRepo.selectMedicoConEspecialidaenFecha(datos.especialidad(), datos.fecha());
